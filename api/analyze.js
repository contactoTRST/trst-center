...
async function checkLLMConsensus(filePath, metadataResult, detectionResult) {
  ...
  try {
    ...
    const prompt = `You are a forensic media authenticity analyst specializing in image manipulation detection. Carefully examine this image for:\n\n1. CUT & PASTE EDGES: Look for hard or soft edges where an element has been cut from another image and pasted in — unnatural halos, fringing, color bleeding, mismatched grain/noise at object boundaries, jagged or overly-smooth selection edges\n2. COMPOSITING ARTIFACTS: Inconsistent lighting direction, mismatched shadows, perspective errors between foreground and background elements\n3. CLONING/RETOUCHING: Repeated texture patterns (copy-move), suspiciously smooth skin or surfaces, smeared or blurred regions\n4. NOISE & GRAIN INCONSISTENCY (KEY SIGNAL): Real camera photos have sensor noise/grain distributed across the entire image — even in skies, skin, and flat backgrounds. Manipulated regions are often suspiciously smooth with clean gradients, while surrounding areas show natural film grain or sensor noise. Look for regions where grain/noise abruptly disappears or changes character, especially around subject edges or background transitions.\n5. AI GENERATION: Unnatural textures, anatomical errors, garbled text, background incoherence throughout the ENTIRE image\n6. METADATA MISMATCH: Evidence supports stripped EXIF — does the image look like it came from a real camera?\n\nCRITICAL VERDICT RULES — read carefully:\n- "manipulated": Use when the image contains REAL photographic elements (real people, real environments, real objects) BUT shows signs of compositing, cut/paste, background replacement, cloning, or selective retouching. This is the most important verdict for editorial fact-checking.\n- "synthetic": ONLY use when the ENTIRE image appears AI-generated with no real photographic source — everything looks generated, not photographed.\n- "likely_synthetic": Use when most of the image appears AI-generated but some elements may be real.\n- If you see a real person in a real setting but the background or other elements look composited/replaced/smoothed — that is "manipulated", NOT "synthetic".\n\nEvidence from other layers: ${evidence}\n\nBe decisive. If ANY cut/paste, cloning, compositing artifacts, or grain/noise inconsistencies are visible around real photographic elements, return "manipulated". Only return "synthetic" if the whole image looks AI-generated from scratch.\n\nRespond in JSON only: {"verdict":"authentic"|"likely_authentic"|"uncertain"|"manipulated"|"likely_synthetic"|"synthetic","confidence":0-100,"flags":["specific artifact observations, e.g. halo around subject at left shoulder, smooth gradient in background vs grainy foreground, mismatched noise at boundary"],"reasoning":"2-3 sentences citing exact visual locations of artifacts"}`;
