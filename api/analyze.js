...
async function checkMetadata(filePath) {
  try {
    ...
    // Screenshots and social media reposts destroy provenance but don't imply fake content.
    // Classify separately so the UI can communicate this distinction clearly.
    let assetType = "original"; // default assumption
    if (fieldCount === 0 && !data.Make && !data.Model) {
      ...
      assetType = "derived"; // some metadata but no camera signature
    }
...
async function checkDetection(filePath) {
  ...
  try {
    ...
    // Threshold: only penalize as high risk at ≥ 0.7. Scores 0.7–0.69 are ambiguous —
...
function scoreToVerdict(score, layers) {
  ...
  if (score >= 40) {
    if (llmVerdict === "synthetic" || llmVerdict === "likely_synthetic") return { label: "Likely AI-Generated", color: "red", recommendation: "Strong signals indicate this image was entirely generated by AI with no real photographic source. TRST does not recommend publishing." };
  ...
  // Distinguish fully AI-generated from other very-low-trust cases
  const isSynthetic = llmVerdict === "synthetic" || llmVerdict === "likely_synthetic";
  if (isSynthetic) return { label: "Likely AI-Generated", color: "red", recommendation: "Strong signals indicate this image was entirely generated by AI with no real photographic source. TRST does not recommend publishing." };
  if (isManipulation) return { label: "Likely Manipulated", color: "red", recommendation: "Strong signals of digital manipulation. Real photo origin likely, but content may have been significantly altered. Do not publish without verification." };
